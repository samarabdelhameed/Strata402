...
> No production code is written. This blueprint is the single source of truth for the MVP architecture.
...
- [ ] `GET /v1/services` exposes full service metadata.
- [ ] Unpaid `POST /v1/strategy/yield-risk` returns `402` with valid machine-readable payment requirements.
- [ ] Consuming agent parses requirements, builds a partially signed HBAR x402 transfer (tinybars, facilitator fee payer), submits via Blocky402 `/verify` + `/settle`.
- [ ] Blocky402 verification + settlement succeed and return a signed settlement result.
- [ ] Retried request with correct `PAYMENT-SIGNATURE` header returns `200` + structured AI analysis + `PAYMENT-RESPONSE` header.
- [ ] Bad proof, wrong amount, wrong recipient, wrong network, expired, and replayed payments are rejected with correct error codes.
- [ ] Deterministic tests for risk score pass (no LLM dependency).
- [ ] A full `e2e` test runs the paid request against testnet and records the Hedera transaction ID.
- [ ] Root README documents setup, architecture, and payment flow.
- [ ] 5-minute demo video recorded showing the live paid request.
...
```
Validation: `accountId` matches `^\d+\.\d+\.\d+$`; `riskTolerance` enum; `amountHbar` > 0. Errors → `422`.
...
**Paid → 200 OK** (+ `PAYMENT-RESPONSE` header with settlement result; `[VERIFY]` exact shape)
```json
{
  "status": "success",
  "service": "strata402-agent",
  "strategy": {
    "name": "Conservative Hedera Yield Strategy",
    "riskScore": 32,
    "confidence": 0.89,
    "recommendation": "Allocate HBAR toward HBAR/USDC stable LPs with tight custody controls."
  },
  "marketContext": {
    "network": "hedera:testnet",
    "saucerSwap": { "status": "adapter-unverified" },
    "bonzo": { "status": "adapter-unverified" },
    "timestamp": "..."
  },
  "payment": {
    "protocol": "x402", "network": "hedera:testnet", "asset": "0.0.0",
    "transactionId": "0.0.1029@..."
  },
...
### Phase 1 — Repository & Bun Workspace Bootstrap + x402 Package Verification (CURRENT)
...
### Phase 7 — Real testnet E2E (qualification-critical)
- Fund buyer via faucet; run `e2e/x402-payment.test`; assert `200` + capture txId + `PAYMENT-RESPONSE`.
- Record demo.

### Phase 8 — Polish
- README completion, docs, dashboard optional, HCS audit wiring, demo video, secret scan, repo publish.
...
- [ ] Real x402 service live on `hedera:testnet` (gateway + engine).
- [ ] `GET /health`, `GET /v1/services` documented and reachable.
- [ ] Unpaid request → `402` with valid requirements.
- [ ] Agent signs real HBAR transfer with official Hedera x402 scheme.
- [ ] Blocky402 `/verify` + `/settle` succeed; settlement response captured.
- [ ] `PAYMENT-SIGNATURE` retry → `200` structured AI analysis + `PAYMENT-RESPONSE` header.
- [ ] Invalid/expired/replayed/wrong-amount/wrong-recipient/wrong-network all rejected.
- [ ] Deterministic risk tests pass; AI fallback verified.
- [ ] Real testnet tx ID recorded in e2e output.
- [ ] Public GitHub repo; secrets clean; `.env.example` placeholder-only.
- [ ] Professional root README done.
- [ ] Demo video ≤ 5 min recorded.
